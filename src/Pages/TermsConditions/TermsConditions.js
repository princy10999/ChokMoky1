...
const TermsConditions = ({ seo, key }) => {
  ...
  useEffect(() => {
    ...
    ApiPost("terms-condition").then((res) => {
      setTerms(res?.data?.result?.terms_condition);
      dispatch(isLoader(false));
    });
  }, []);
  return (
    <Box key={key}>
      <SEOPart data={SeoData?.terms} seo={seo} />
